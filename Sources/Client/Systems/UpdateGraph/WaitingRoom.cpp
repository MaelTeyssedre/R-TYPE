...
{
    _execScene[rtype::constants::WAITING_ROOM] = std::function(
        [this](Registry &r, SparseArray<components::currentScene_s> &currentScenes) -> void
        {
            ...
            std::vector<constants::TYPE> type;
            ...
            for (auto &&[pos, sprite, scene, drawable, index, zaxis] : Zipper(positions, sprites, scenes, drawables, indexs, zaxises))
            {
                ...
                type.push_back(constants::TYPE::SPRITE);
            ...
            for (auto &&[text, drawable, color, pos, scene, zaxis, index] : Zipper(texts, drawables, colors, positions, scenes, zaxises, indexs))
            {
                ...
                myZAxises.push_back((int)zaxis.zAxis);
                zAxisMap[(int)zaxis.zAxis] = index.idx;
                type.push_back(constants::TYPE::TEXT);
            ...
            {
                if (type[i] == constants::TYPE::SPRITE)
                    _graphicalLib->draw(zAxisMap[i]);
                if (type[i] == constants::TYPE::TEXT)
                    _graphicalLib->print(zAxisMap[i]);
            }
